...
describe('TabBar display mode logic', () => {
  ...
  test('at 140 cols (full mode), uses full labels', () => {
    ...
    // Check that "board" suffix appears (from "Dashboard" after wrap)
    // #1754: With 17 tabs, layout wraps differently - check for "board" and "[1]"
  ...
  test('at 120 cols (full mode boundary), uses full labels', () => {
    ...
    // #1754: With 17 tabs, "Dashboard" wraps as "Das" + "board"
...
describe('TabBar structure', () => {
  ...
  test('full labels map correctly at 120+ cols', () => {
    ...
    // #1754: With 17 tabs, wrapping differs - check for key parts
    expect(output).toContain('[1]');
    expect(output).toContain('[2]');
    expect(output).toContain('[3]');
    expect(output).toContain('[4]');
    // With 17 tabs at 80-col render width, labels wrap - check for presence
    expect(output).toMatch(/Ag/); // "Agents" may wrap
  });
