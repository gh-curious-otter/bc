...
describe('MetricCard', () => {
  describe('basic rendering', () => {
    it('renders label and value', () => {
      const { lastFrame } = render(<MetricCard label="Total" value={42} />);
    ...
    it('renders with zero value', () => {
      const { lastFrame } = render(<MetricCard label="Errors" value={0} />);
      const output = lastFrame() ?? '';
    ...
    it('renders with string value', () => {
      const { lastFrame } = render(<MetricCard label="Status" value="OK" />);
      const output = lastFrame() ?? '';
    ...
    it('renders large numbers', () => {
      const { lastFrame } = render(<MetricCard label="Tokens" value={1234567} />);
      const output = lastFrame() ?? '';
  ...
  describe('colors', () => {
    it('renders with custom color', () => {
      const { lastFrame } = render(<MetricCard label="Active" value={5} color="green" />);
      const output = lastFrame() ?? '';
    ...
    it('renders with red color', () => {
      const { lastFrame } = render(<MetricCard label="Failed" value={3} color="red" />);
      expect(lastFrame()).toContain('Failed');
    });

    it('renders with cyan color', () => {
      const { lastFrame } = render(<MetricCard label="Progress" value={50} color="cyan" />);
  ...
  describe('prefix and suffix', () => {
    it('renders with prefix', () => {
      const { lastFrame } = render(<MetricCard value={99} label="Cost" prefix="$" />);
      expect(lastFrame()).toContain('$99');
    });

    it('renders with suffix', () => {
      const { lastFrame } = render(<MetricCard value={75} label="Success" suffix="%" />);
  ...
  describe('value types', () => {
    ...
    it('renders very large numbers', () => {
      const { lastFrame } = render(<MetricCard value={999999999} label="Big" />);
  ...
  describe('edge cases', () => {
    ...
    it('renders with special characters', () => {
      const { lastFrame } = render(<MetricCard value={100} label="Test [#123]" />);
