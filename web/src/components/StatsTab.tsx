...
export function StatsTab({ agent }: { agent: Agent }) {
  ...
  const memPct = latest && latest.mem_limit_mb > 0
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        <h2 className="text-sm font-medium text-bc-muted uppercase tracking-wide">Cost Breakdown</h2>
      ...
      {/* Resource Usage */}
      <div className="space-y-2">
        <h2 className="text-sm font-medium text-bc-muted uppercase tracking-wide">Resource Usage</h2>
        <div className="rounded border border-bc-border bg-bc-surface p-4">
          {is404 ? (
            ...
            <p className="text-sm text-bc-error">Failed to load stats: {error}</p>
          ) : !latest ? (
            <p className="text-sm text-bc-muted italic">No resource samples collected yet</p>
          ) : (
            <div className="space-y-4">
              ...
              {latest && (
                <div className="grid grid-cols-2 gap-2 text-xs text-bc-muted pt-2 border-t border-bc-border/30">
                  <span>Mem: {latest.mem_used_mb.toFixed(1)} / {latest.mem_limit_mb.toFixed(1)} MB</span>
                  <span>Net RX: {latest.net_rx_mb.toFixed(2)} MB</span>
                  <span>Net TX: {latest.net_tx_mb.toFixed(2)} MB</span>
                  <span>Block R/W: {latest.block_read_mb.toFixed(2)} / {latest.block_write_mb.toFixed(2)} MB</span>
                </div>
              )}
            </div>
          )}
      ...
      {/* Session Info */}
      <div className="space-y-2">
        <h2 className="text-sm font-medium text-bc-muted uppercase tracking-wide">Session Info</h2>
        <div className="rounded border border-bc-border bg-bc-surface p-4 space-y-2">
          <div className="flex items-start gap-2 py-1.5 border-b border-bc-border/30">
