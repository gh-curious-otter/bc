...
export function Daemons() {
  ...
  const { data: daemons, loading, error, refresh, timedOut } = usePolling(fetcher, 10000);
  const [actionLoading, setActionLoading] = useState<string | null>(null);
  const [confirmAction, setConfirmAction] = useState<{ name: string; action: 'stop' | 'restart' | 'remove' } | null>(null);

  const performAction = useCallback(async (name: string, action: 'stop' | 'restart' | 'remove') => {
  ...
  const columns = [
    ...
    },
    {
      key: 'runtime',
      label: 'Runtime',
      render: (d: Daemon) => <code className="text-xs text-bc-muted">{d.runtime}</code>,
    },
    {
      key: 'status',
      label: 'Status',
    ...
    },
    {
      key: 'started',
      label: 'Started',
      render: (d: Daemon) => <span className="text-xs text-bc-muted">{timeAgo(d.started_at)}</span>,
    },
    {
      key: 'actions',
      label: 'Actions',
        ...
        <div className="flex items-center gap-2">
          {d.status === 'running' && (
            <button
              ...
              onClick={(e) => { e.stopPropagation(); setConfirmAction({ name: d.name, action: 'stop' }); }}
          ...
          <button
            type="button"
            disabled={actionLoading !== null}
            onClick={(e) => { e.stopPropagation(); setConfirmAction({ name: d.name, action: 'restart' }); }}
          ...
          <button
            type="button"
            disabled={actionLoading !== null}
            onClick={(e) => { e.stopPropagation(); setConfirmAction({ name: d.name, action: 'remove' }); }}
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <span className="text-sm text-bc-muted">{daemons?.length ?? 0} daemons</span>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-bc-surface border border-bc-border rounded-lg p-6 max-w-sm w-full mx-4 space-y-4">
            ...
            <p className="text-sm text-bc-muted">
              ...
              <span className="font-medium text-bc-text">{confirmAction.name}</span>?
            ...
            <div className="flex justify-end gap-2">
              ...
              <button
                type="button"
                disabled={actionLoading !== null}
                onClick={() => performAction(confirmAction.name, confirmAction.action)}
              ...
              >
                {actionLoading ? 'Working...' : confirmAction.action.charAt(0).toUpperCase() + confirmAction.action.slice(1)}
              </button>
            </div>
          </div>
        </div>
