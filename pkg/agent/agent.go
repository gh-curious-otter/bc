...
// Default configuration constants (formerly generated by cfgx from settings.toml).
const (
	...
	if agentRuntime == "docker" {
		...
		if p, ok := m.providerRegistry.Get(effectiveTool); ok {
			if !p.IsInstalled(ctx) {
				m.mu.Unlock()
				return nil, fmt.Errorf("tool %q is not installed. Install %s or configure a different tool in settings.toml", effectiveTool, p.Name())
			}
	...
	if !providerValidated && agentCmd != "" {
		...
		if len(parts) > 0 {
			if _, err := exec.LookPath(parts[0]); err != nil {
				m.mu.Unlock()
				return nil, fmt.Errorf("tool %q command %q not found in PATH. Install it or configure a different tool in settings.toml", effectiveTool, parts[0])
			}
		}
	}
